...
def searchDoctors(request):
    ...
    doctors = Doctor_Information.objects.filter(register_status='Accepted').distinct().filter(
        ...
        Q(appointed_hospitals__name__icontains=search_query) |
...
def paginateDoctors(request, doctors, results):

    page = request.GET.get('page')
    paginator = Paginator(doctors, results)
...
def searchDepartmentDoctors(request, pk):
    ...
    doctors = Doctor_Information.objects.filter(register_status='Accepted', department_name=departments).filter(
        Q(name__icontains=search_query))
